...
async def handle_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "menu_newgiveaway":
        ...
        text = (
            f"ℹ️ *{f('About This Bot')}*\n\n"
            f"╔══════════════════════╗\n"
            f"║  🎉  *{f('GIVEAWAY BOT')}*  ║\n"
            f"╚══════════════════════╝\n\n"
            f"*{f('Version')}:* 1.0\n\n"
            f"━━━━━━━━━━━━━━━━━━━━━━\n"
            f"🎯 *{f('What can this bot do?')}*\n\n"
            f"• {f('Create live giveaways in your channel')}\n"
            f"• {f('Free voting + paid voting system')}\n"
            f"• {f('Real-time leaderboard')}\n"
            f"• {f('Auto milestone announcements')}\n"
            f"• {f('Participant vote notifications')}\n"
            f"• {f('Channel leave vote detection')}\n\n"
            f"━━━━━━━━━━━━━━━━━━━━━━\n"
            f"_{f('For support, updates or to contact the owner')}_\n"
            f"_{f('use the buttons below')} 👇_"
        )
        kb = InlineKeyboardMarkup([
            [
