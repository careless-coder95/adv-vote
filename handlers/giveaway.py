...
async def _check_milestone(context, giveaway: dict, participant: dict, total: int):
    """Vote milestone reach hone par channel mein special announcement + auto-end check."""
    giveaway_id = giveaway["giveaway_id"]

    # ── Auto-end check — min_votes reach hone par automatic end ──
    min_votes = giveaway.get("min_votes", 0)
    if min_votes:
        board       = [p for p in participants_col.find({"giveaway_id": giveaway_id, "verified": True})]
        ...
        if total_all >= min_votes and giveaway["status"] == "active":
            ...
            return  # Milestone announce ki zaroorat nahi, end announcement aa jayegi
...
async def handle_join_link(update: Update, context: ContextTypes.DEFAULT_TYPE, giveaway_id: str):
    ...
    if existing and existing.get("verified"):
        # ── Better duplicate message — show full details ──
...
async def handle_verify_submit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    existing = get_participant(giveaway_id, user.id)
    if existing and existing.get("verified"):
        # ── Better duplicate on verify button too ──
        paid        = giveaway["voting_type"] in ("paid", "both")
        total_votes = get_total_votes(giveaway_id, existing["participant_id"])
    ...
    success_text = (
        ...
        f"🔗 *{f('Your Vote Post')}:* {post_link if post_link else f('(unavailable)')}\n\n"
...
async def handle_free_vote(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """One vote per user — change allowed, old vote auto-removed."""
    query          = update.callback_query
    ...
    # ── Channel membership check — show join button if not member ──
        ...
        try:
            await context.bot.send_message(
                ...
                text=(
                    ...
                    f"_{f('Join the channel first, then come back and vote.')}_"
                ),
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton(
                        f"📢 {f('Join Channel')}",
                        url=giveaway.get("target_link", "")
                    )
                ]]),
            )
        except: pass
    ...
    # ── Notify participant with voter name ──
    if participant:
        try:
            voter_display = f"@{voter_username}" if query.from_user.username else voter_username
            await context.bot.send_message(
                chat_id=participant["user_id"],
                text=(
                    f"🗳️ *{f('New Vote Received!')}*\n\n"
                    f"👤 *{f('From')}:* {voter_display}\n"
