...
from utils import (
    get_giveaway, get_participant, check_channel_membership,
    format_participant_post, build_vote_button,
    build_participant_bot_buttons, random_emoji, get_total_votes,
)
...
async def handle_join_link(update: Update, context: ContextTypes.DEFAULT_TYPE, giveaway_id: str):
    ...
    if not giveaway or giveaway["status"] != "active":
        return await update.message.reply_text(
            f"❌ *{f('This giveaway is not active or does not exist.')}*",
            parse_mode="Markdown"
        )

    if giveaway.get("paused"):
        return await update.message.reply_text(
            f"⏸️ *{f('This giveaway is currently paused.')}*\n"
    ...
    if existing and existing.get("verified"):
        ...
        return await update.message.reply_text(
            f"✅ *{f('You are already participating in this giveaway!')}*\n\n"
            f"🎉 {giveaway.get('channel_name', '')}",
            parse_mode="Markdown",
            reply_markup=build_participant_bot_buttons(
                giveaway_id, existing["participant_id"],
                existing.get("post_link", ""), paid
            ),
        )
    ...
    caption = (
        ...
        + f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"👇 *{f('Steps')}:*\n"
        f"1️⃣ {f('Click')} *{f('Join Channel')}* {f('below')}\n"
        f"2️⃣ {f('Join the channel')}\n"
        f"3️⃣ {f('Click')} *{f('Verify & Submit')}*\n"
        f"━━━━━━━━━━━━━━━━━━━━━━"
    )
...
async def handle_verify_submit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if giveaway.get("paused"):
        return await query.answer(f("Giveaway is paused. Try again later."), show_alert=True)

    joined = await check_channel_membership(context.bot, giveaway["target_username"], user.id)
    if not joined:
        return await query.answer(f("Please join the channel first, then verify!"), show_alert=True)
    ...
    if existing and existing.get("verified"):
        ...
        try:
            ...
            pass
        return await query.answer(f("You are already a participant!"), show_alert=True)
    ...
    try:
        await query.edit_message_text(
            ...
            f"{emoji} {f('Your post is live in the channel!')}\n\n"
            f"━━━━━━━━━━━━━━━━━━━━━━\n"
            f"{f('Use the buttons below')} 👇",
            parse_mode="Markdown",
...
async def handle_free_vote(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    giveaway = get_giveaway(giveaway_id)
    if not giveaway or giveaway["status"] != "active":
        return await query.answer(f("This giveaway is not active."), show_alert=True)
...
async def handle_channel_leave(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for giveaway in active_giveaways:
        ...
        logger.info(f"User {user_id} left → free vote removed from {gid}")
