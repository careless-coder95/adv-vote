...
async def start_create(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["cg"] = {"step": STEP_DESC, "creator_id": update.effective_user.id}
    text = (
    ...
    try: await update.callback_query.edit_message_caption(text, parse_mode="Markdown", reply_markup=_cancel_kb())
    except: await update.effective_message.reply_text(text, parse_mode="Markdown", reply_markup=_cancel_kb())
...
async def handle_create_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if step == STEP_DESC:
        if msg.photo:
            ...
            await msg.reply_text(f"❌ {f('Send text or photo.')}")
            return True
        data["step"] = STEP_CH
        ...
        try:
            ...
            await msg.reply_text(f"❌ {f('Send a positive number. Example')}: `5`", parse_mode="Markdown")
            return True
        data["votes_per_rupee"] = rate
        data["step"]            = STEP_MIN
        await msg.reply_text(
            f"✅ *{f('Rate saved!')}* ₹1 = {rate} {f('votes')}\n\n"
            f"🎯 *{f('Minimum Votes to End')}*\n\n"
...
async def handle_create_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "create_cancel":
        ...
        await query.edit_message_text(f"❌ {f('Cancelled. Use /start to go back.')}")
        return
...
async def _finalize(update, context, data):
    gid          = gen_giveaway_id()
    ...
    vmap = {"free": f"🆓 {f('Free Only')}", "paid": f"💰 {f('Paid Only')}", "both": f"🔀 {f('Free + Paid')}"}
    text = (
        ...
        + (f"🎯 *{f('Min Votes')}:* {data.get('min_votes',0)}\n" if data.get('min_votes') else "")
        + f"━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"🔗 *{f('Join Link')}:*\n`{join_link}`\n\n"
