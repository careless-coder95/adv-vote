...
async def do_delete(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Giveaway My Giveaways list se remove karo."""
    query       = update.callback_query
    await query.answer()
    giveaway_id = query.data.split("|")[1]
    giveaway    = get_giveaway(giveaway_id)
    ...
    # Sirf giveaway record delete karo — votes/payments history rahegi
    giveaways_col.delete_one({"giveaway_id": giveaway_id})

    await _send_or_edit(
        query,
        f"🗑️ *{f('Giveaway deleted from your list!')}*\n\n"
        f"_{f('Channel posts are still there.')}_",
...
async def _do_auto_end(context, giveaway: dict):
    """Min votes reach hone par automatic end — creator ko notify karo."""
    ...
    try:
        await context.bot.send_message(
            ...
            text=(
                f"🏁 *{f('Giveaway Auto-Ended!')}*\n\n"
                f"📡 *{f('Giveaway')}:* {giveaway.get('channel_name', '')}\n"
                f"🎯 *{f('Reason')}:* {f('Minimum vote target reached!')}\n\n"
                f"{result}"
            ),
            parse_mode="Markdown",
        )
...
async def show_giveaway_detail(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if giveaway["status"] == "active":
        ...
        # Ended giveaway — sirf Stats aur Delete
        btns.append([
            InlineKeyboardButton(f"📊 {f('Stats')}", callback_data=f"gwstats|{giveaway_id}"),
...
async def confirm_end(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Show current progress if min_votes is set — info only, not a block
    ...
    if min_votes:
        ...
        extra = (
            f"\n━━━━━━━━━━━━━━━━━━━━━━\n"
            f"🎯 *{f('Min Votes Target')}:* {min_votes}\n"
