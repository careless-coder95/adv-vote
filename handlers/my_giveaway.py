...
async def show_my_giveaways(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for g in giveaways:
        ...
        closed  = " 🔒" if g.get("participation_closed") else ""
        p_count = participants_col.count_documents({"giveaway_id": g["giveaway_id"], "verified": True})
        label   = f"{icon}{paused}{closed} {g.get('channel_name', g['giveaway_id'])} ({p_count})"
        text   += f"{icon}{paused}{closed} *{g.get('channel_name', g['giveaway_id'])}* — {p_count} {f('participants')}\n"
...
async def show_giveaway_detail(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if giveaway["status"] == "active":
        ...
        part_lbl  = f"🔓 {f('Open Participation')}" if p_closed else f"🔒 {f('Close Participation')}"
        btns.append([
        ...
        btns.append([
            InlineKeyboardButton(part_lbl, callback_data=f"gwpart|{giveaway_id}"),
        ])
        btns.append([
        ...
        btns.append([
            InlineKeyboardButton(f"📊 {f('Stats')}",   callback_data=f"gwstats|{giveaway_id}"),
            InlineKeyboardButton(f"🗑️ {f('Delete')}", callback_data=f"gwdelete_confirm|{giveaway_id}"),
        ])
...
async def toggle_participation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Close/Open participation — no new participants allowed when closed."""
    query       = update.callback_query
    await query.answer()
    giveaway_id = query.data.split("|")[1]
    giveaway    = get_giveaway(giveaway_id)
    ...
    new_closed = not giveaway.get("participation_closed", False)
    giveaways_col.update_one({"giveaway_id": giveaway_id}, {"$set": {"participation_closed": new_closed}})

    msg = f"🔒 {f('Participation closed! No new participants.')}" if new_closed else f"🔓 {f('Participation opened!')}"
    await query.answer(msg, show_alert=True)
    query.data = f"mygw|{giveaway_id}"
    await show_giveaway_detail(update, context)
...
async def delete_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    giveaway    = get_giveaway(giveaway_id)
    if not giveaway or giveaway["creator_id"] != query.from_user.id:
        return await query.answer(f("Only the creator can delete."), show_alert=True)

    text = (
        f"🗑️ *{f('Delete Giveaway?')}*\n\n"
        f"📡 *{f('Giveaway')}:* {giveaway.get('channel_name', '')}\n\n"
        f"_{f('This will remove it from your list.')}_\n"
        f"_{f('Channel posts will not be deleted.')}_"
    )
    kb = InlineKeyboardMarkup([[
        InlineKeyboardButton(f"✅ {f('Yes, Delete')}", callback_data=f"gwdelete_do|{giveaway_id}"),
...
async def do_delete(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if giveaway["status"] != "ended":
        return await query.answer(f("Only ended giveaways can be deleted."), show_alert=True)

    giveaways_col.delete_one({"giveaway_id": giveaway_id})
    await _send_or_edit(
        query,
        f"🗑️ *{f('Giveaway deleted from your list!')}*\n\n_{f('Channel posts are still there.')}_",
...
async def confirm_end(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if min_votes:
        ...
        extra = (
            f"\n━━━━━━━━━━━━━━━━━━━━━━\n"
            f"🎯 *{f('Auto-End Target')}:* {min_votes}\n"
            f"📊 *{f('Current Total')}:* {total_votes}\n"
        )
...
async def do_end(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    result = await _do_end_logic(context, giveaway)
...
async def _do_end_logic(context, giveaway: dict) -> str:
    """Core end logic — used by both manual end and auto-end."""
...
async def _do_auto_end(context, giveaway: dict):
    """Auto end when min votes reached."""
    result = await _do_end_logic(context, giveaway)
    revenue = sum(p.get("amount", 0) for p in payments_col.find({"giveaway_id": giveaway["giveaway_id"], "status": "approved"}))

    auto_result = (
        f"🏁 *{f('GIVEAWAY AUTO-ENDED!')}*\n"
        f"_{f('Minimum vote target reached!')}_\n\n"
        + result
    )
    ...
    try:
        await context.bot.send_message(
            ...
            text=auto_result + (f"\n\n💰 *{f('Total Revenue')}: ₹{revenue}*" if revenue else ""),
            parse_mode="Markdown",
        )
    except: pass
