...
async def handle_buy_votes_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        ...
        text=(
            ...
            f"━━━━━━━━━━━━━━━━━━━━━━\n"
            f"{f('How much amount do you want to pay?')}\n"
            f"{f('Send a number in')} ₹ ({f('Example')}: `50`)\n\n"
            f"_₹50 × {rate} = {50 * rate} {f('votes')}_"
        ),
        parse_mode="Markdown",
    ...
    try:
        await query.answer(f("Check bot DM 👆"), show_alert=True)
    except Exception:
        pass
...
async def handle_paid_vote_amount(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    try:
        ...
        await update.message.reply_text(
            f"❌ {f('Please send a positive number. Example')}: `100`",
            parse_mode="Markdown"
        )
        return True
    ...
    await update.message.reply_text(
        ...
        f"━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"{f('Pay using the QR below, then send screenshot here.')} 👇",
        parse_mode="Markdown",
    )
    ...
    if giveaway and giveaway.get("qr_file_id"):
        await context.bot.send_photo(
            ...
            caption=(
                f"📲 *{f('Pay')} ₹{amount}*\n"
                f"{f('TXN ID')}: `{txn_id}`\n\n"
                f"✅ {f('After payment, send screenshot here.')}"
            ),
            parse_mode="Markdown",
        )
    return True
...
async def handle_paid_vote_screenshot(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    admin_text = (
        ...
        f"━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"{f('Take action below')} 👇"
    )
    approve_kb = InlineKeyboardMarkup([[
        InlineKeyboardButton(f"✅ {f('Approve')}", callback_data=f"paidapprove|{txn_id}"),
        InlineKeyboardButton(f"❌ {f('Decline')}", callback_data=f"paiddecline|{txn_id}"),
    ]])
    ...
    await update.message.reply_text(
        ...
        f"🧾 *{f('TXN ID')}:* `{txn_id}`\n"
        f"_{f('The giveaway creator will verify and update you shortly.')}_ 🙏",
        parse_mode="Markdown",
    )
...
async def handle_approve_decline(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not giveaway or giveaway["creator_id"] != query.from_user.id:
        return await query.answer(f("Only the giveaway creator can approve/decline."), show_alert=True)
    ...
    if action == "paidapprove":
        ...
        if p and p.get("post_message_id") and giveaway.get("status") == "active":
            try:
                await context.bot.edit_message_reply_markup(
                    ...
                    reply_markup=build_vote_button(
                        payment["giveaway_id"], payment["participant_id"], p["emoji"]
                    ),
                )
            except Exception:
                pass
        ...
        try:
            ...
            await context.bot.send_message(
                ...
                text=(
                    ...
                    f"🗳️ *{f('Votes Added')}:* {votes_to_add}\n"
                    f"📊 *{f('Total Now')}:* {total_now}\n\n"
                    f"_{f('Buy paid votes to support your favourite!')}_"
                ),
                parse_mode="Markdown",
            )
        except Exception:
        ...
        try:
            await context.bot.send_message(
                ...
                text=(
                    ...
                    f"🎯 *{f('Candidate')}:* {payment['participant_name']}\n"
                    f"🗳️ *{f('Votes Added')}:* {votes_to_add}\n\n"
                    f"✅ {f('Vote has been added to the channel!')}"
                ),
                parse_mode="Markdown",
            )
        except Exception:
        ...
        try:
            await query.edit_message_caption(
                (query.message.caption or "") + f"\n\n✅ *{f('APPROVED')} — {votes_to_add} {f('votes added!')}*",
                parse_mode="Markdown",
            )
        except Exception:
            pass
        ...
        context.user_data["declining_txn"]           = txn_id
        await query.message.reply_text(
            f"❌ {f('Declining TXN')} `{txn_id}`\n\n"
            f"{f('Send the decline reason (will be sent to user)')}:",
            parse_mode="Markdown",
        )
...
async def handle_decline_reason(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    await update.message.reply_text(
        f"❌ {f('TXN')} `{txn_id}` {f('has been declined.')}",
        parse_mode="Markdown"
    )
